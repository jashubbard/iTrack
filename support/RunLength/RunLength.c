...
/*
% $JRev: R-t V:020 Sum:KKDOqYj3EVtq Date:17-Sep-2013 00:46:24 $
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (mxGetM(prhs[0]) == n1) {
     out_1 = n1;                        // Reply a column vector
     out_2 = 1;
  } else if (mxGetNumberOfDimensions(prhs[0]) == 2 && mxGetN(prhs[0]) == n1) {
     out_1 = 1;                         // Reply a row vector
     out_2 = n1;
  } else {
     ERROR("BadSizeInput1", "Data must be a row or column vector.");
  }
  isColumn = (bool) (out_1 > out_2);    // Prefer columns for scalar input
  ...
  if (doEncode) {
     ...
     // Create output:
     plhs[0] = mxCreateNumericMatrix(out_1, out_2, classIn1, mxREAL);
     if (doByte) {
        plhs[1] = mxCreateNumericMatrix(1, nx, mxUINT8_CLASS, mxREAL);
     } else {
  ...
  } else {  // !doEncode, Inputs and outputs for decoding:
     // Get input:
     b  = mxGetData(prhs[0]);
     nb = n1;
     n  = mxGetData(prhs[1]);
  ...
  if (doEncode)  {  // Encode [x]->[b,n]: --------------------------------------
     ...
     // Crop unneeded pre-allocated elements - mxRealloc is *not* mandatory and
     // under some circumstances it even does not free the memory in modern
     // Matlab versions:
     if (nb != nx) {
        if (isColumn) {
           mxSetM(plhs[0], nb);
     ...
     // Create 3rd output as post-processing - collecting the indices on the fly
     // would be faster, but the number of subfunctions would be doubled.
     if (nlhs == 3) {
